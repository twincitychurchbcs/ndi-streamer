 ...
 */
CodecFrameRate VAAPIDecoder::GetFrameRate() {
    FUNCTION_CALL_DEBUG();
 ...
 * @return AvException
 */
AvException VAAPIDecoder::FillVAAPIDecoder(AVPacket *packet) {
    FUNCTION_CALL_DEBUG();
 ...
 */
VAAPIDecoderOutput VAAPIDecoder::Decode() {
    FUNCTION_CALL_DEBUG();
#ifdef _DEBUG
    // Profile function
    ...
    DEBUG("VAAPI Decode time (seconds): %f", std::chrono::duration<double>(time_end - time_start).count());
#endif
 ...
 */
VAAPIDecoderResult VAAPIDecoder::Create(AVCodecParameters *codecpar) {
    FUNCTION_CALL_DEBUG();
    ...
    try {
        return {std::unique_ptr<VAAPIDecoder>(new VAAPIDecoder(codecpar)), error};
    } catch (AvException e) {
        error = e;
        DEBUG("VAAPIDecoder error: %s", error.what());
    }
...
/**
 * @brief Construct a new VAAPIDecoder:: VAAPIDecoder object
 *
 * @param codec_id codec ID
 */
VAAPIDecoder::VAAPIDecoder(AVCodecParameters *codecpar) : m_codecpar(codecpar) {
    FUNCTION_CALL_DEBUG();
 ...
 * @return AvError
 */
AvError VAAPIDecoder::m_Initialize() {
    FUNCTION_CALL_DEBUG();

    // Create hardware device
    int ret = av_hwdevice_ctx_create(&m_hw_device_ctx, AV_HWDEVICE_TYPE_VAAPI, nullptr, nullptr, 0);
    if (ret < 0) {
